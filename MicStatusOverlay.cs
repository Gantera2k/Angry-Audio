...
namespace AngryAudio
{
    public class MicStatusOverlay : Form
    {
        ...
        private float _shimmerFadeTarget; // opacity to fade to when single sweep completes
            ...
            _shimmerTimer.Tick += (s, e) => {
                ...
                if (_shimmerX > 1.3f)
                {
                    if (_shimmerFinishing)
                    {
                        ...
                        _shimmerTimer.Stop();
                        _shimmerX = -0.3f;
                        // If not held, kick off fade immediately — don't wait for settle/dim timer
                        if (!_pttHeld)
                        {
                            _settleTimer.Stop();
                            _dimTimer.Stop();
        ...
        public void UseExtendedDelay() { /* no-op: uniform 1s display */ }
        ...
        public void ShowMicOpen() {
            ...
            if (ToggleMode || PushToMuteMode) {
                // Toggle or PTM: no held state. Flash bright, single shimmer sweep, then fade to dim.
        ...
        public void ShowMicClosed() {
            ...
            if (PushToMuteMode) {
                ...
                _shimmerActive = true; _shimmerFinishing = false; // continuous
                _pttHeld = true;
                Action a = () => {
                    if (_opacity < 0.01f) { SizeToContentSafe(); Position(); _opacity = 0f; }
                    _targetOpacity = BRIGHT;
            ...
            if (ToggleMode) {
                // Toggle: flash bright, single shimmer sweep, then fade to nothing.
            ...
            // Normal PTT: released — flash, single shimmer sweep, then fade to nothing.
            _shimmerActive = false; _shimmerFinishing = true; _shimmerFadeTarget = 0f;
            _pttHeld = false;
            Action a2 = () => {
                if (_opacity < 0.01f) { SizeToContentSafe(); Position(); _opacity = 0f; }
        ...
        /// <summary>Mic open in idle/dim state (non-PTT apps using mic). Appears bright with single shimmer, fades to dim.</summary>
        public void ShowMicOpenIdle() {
            if (!OverlayEnabled) return;
            _micOpen = true; _pttHeld = false;
            _shimmerActive = false; _shimmerFinishing = true; _shimmerFadeTarget = DIM; // single sweep → dim
            _settleTimer.Stop();
            _shimmerX = -0.3f;
            _dimTimer.Stop();
            Action a = () => {
