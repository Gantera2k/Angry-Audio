...
namespace AngryAudio
{
    ...
    public static class ToastStack
    {
        ...
        private const int Gap = 8; // pixels between toasts
        ...
        public static void Register(Form toast)
        {
            lock (_lock)
            {
                ...
                // Enforce maximum 1 toast on screen at once — prevents lag from rapid cycling
                while (_toasts.Count >= 1)
                {
                    var oldest = _toasts[0];
                    _toasts.RemoveAt(0);
                    _targetY.Remove(oldest);
                    try
                    {
                        var dismiss = oldest.GetType().GetMethod("Dismiss", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
                        if (dismiss != null) dismiss.Invoke(oldest, null);
                        else oldest.Close();
                    }
                    catch { try { oldest.Close(); } catch { } }
                }
