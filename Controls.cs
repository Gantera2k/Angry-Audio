...
namespace AngryAudio
{
    ...
    public class SlickSlider : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e) {
            ...
            // Filled portion
            if (tx > tl + 2) {
                Color fillBase = Color.FromArgb((int)(255 * dim), FILL_CLR.R, FILL_CLR.G, FILL_CLR.B);
                Color fillLight = Color.FromArgb((int)(255 * dim), Math.Min(255, FILL_CLR.R + 40), Math.Min(255, FILL_CLR.G + 40), Math.Min(255, FILL_CLR.B + 40));
                using (var b = new System.Drawing.Drawing2D.LinearGradientBrush(new Point(tl, ty - th/2), new Point(tl, ty + th/2), fillLight, fillBase))
            ...
            Rectangle thumbRect = new Rectangle(tx - thr, ty - thr, thr * 2, thr * 2);
            if (thumbRect.Width > 0 && thumbRect.Height > 0) {
                using (var b = new System.Drawing.Drawing2D.LinearGradientBrush(thumbRect, Color.FromArgb(thumbAlpha, 255, 255, 255), Color.FromArgb(thumbAlpha, 210, 210, 210), 90f))
                    g.FillEllipse(b, thumbRect);
                if (Enabled) {
                    using (var p = new Pen(Color.FromArgb(50, 0, 0, 0), 1f))
                        g.DrawEllipse(p, thumbRect);
                    using (var p = new Pen(Color.White, 1f))
                        g.DrawEllipse(p, tx - thr + 1, ty - thr + 1, thr * 2 - 2, thr * 2 - 2);
                }
            }
    ...
    class ScrollPanel : Panel
    {
        ...
        protected override void WndProc(ref Message m) {
            base.WndProc(ref m);
            if (m.Msg == 0x85 || m.Msg == 0x14 || m.Msg == 0x0F) { // WM_NCPAINT, WM_ERASEBKGND, WM_PAINT
                ShowScrollBar(Handle, 1, false); // SB_VERT
            }
        }
    }
    ...
    /// <summary>CheckBox that draws ONLY its icon with no background.</summary>
    public class TransparentCheckBox : CheckBox
    {
        ...
        protected override void OnPaintBackground(PaintEventArgs e)
        {
            // Skip — let parent content show through if needed
        }
        protected override void OnPaint(PaintEventArgs e)
        {
    ...
    public class CardIcon
    {
        ...
        public bool IsDuck; // true = rubber duck icon for audio ducking
        public bool IsKey; // true = skeleton key icon for hotkey suppression
        public bool Visible = true; // if false, skip painting and hit-testing
        public bool Dimmed  = false; // if true, paint at reduced opacity, no click
        public Action<bool> OnChange;
        public Action OnRightClick; // right-click action (e.g. navigate to settings page)
        public Panel Card;
        ...
        public bool HitTest(Point p) { return Visible && !Dimmed && Bounds.Contains(p); }

        private DateTime _lastToggle = DateTime.MinValue;
        public void Toggle() {
            if ((DateTime.UtcNow - _lastToggle).TotalMilliseconds < 300) return; // 300ms cooldown — duck fade is ~100ms
            _lastToggle = DateTime.UtcNow;
            Checked = !Checked; if (OnChange != null) OnChange(Checked); if (Card != null) Card.Invalidate();
        }
        ...
        public void Paint(Graphics g, Color accent) {
            ...
            Color col = Dimmed ? Color.FromArgb(45, 45, 45)
                : Checked ? (Hover ? Color.FromArgb(140, 220, 255) : accent) : (Hover ? Color.FromArgb(90, 90, 90) : Color.FromArgb(55, 55, 55));
            float sc = Dpi.S(1);
            if (IsLock) PaintLock(g, cx, cy, Checked && !Dimmed, col, sc);
            else if (IsDuck) PaintDuck(g, cx, cy, Checked && !Dimmed, col, sc);
            else if (IsKey) PaintKey(g, cx, cy, Checked && !Dimmed, col, sc);
        ...
        static void PaintDuck(Graphics g, float cx, float cy, bool on, Color col, float sc) {
            // Rubber duck silhouette
            g.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
            float bw = 10 * sc, bh = 6.5f * sc;
            float bx = cx - bw * 0.55f, by = cy - bh * 0.3f;
            using (var b = new SolidBrush(col)) g.FillEllipse(b, bx, by, bw, bh);
            using (var b = new SolidBrush(col)) g.FillEllipse(b, bx - 1.5f * sc, by + bh * 0.15f, 3.5f * sc, 4 * sc);
            float hr = 3.2f * sc;
            float hx = cx + 1.5f * sc, hy = cy - 3.5f * sc;
            using (var b = new SolidBrush(col)) g.FillEllipse(b, hx - hr, hy - hr, hr * 2, hr * 2);
            Color beakCol = col; // Same color as body — no variation
            var beak = new PointF[] { new PointF(hx + hr * 0.5f, hy - 1.2f * sc), new PointF(hx + hr + 2.5f * sc, hy + 0.2f * sc), new PointF(hx + hr * 0.5f, hy + 1.5f * sc) };
            using (var b = new SolidBrush(beakCol)) g.FillPolygon(b, beak);
            if (on) { float er = 0.7f * sc; using (var b = new SolidBrush(Color.FromArgb(30, 30, 30))) g.FillEllipse(b, hx + 0.2f * sc - er, hy - 0.8f * sc - er, er * 2, er * 2); }
            if (on) { using (var p = new Pen(Color.FromArgb(40, 255, 255, 255), 0.7f * sc)) g.DrawArc(p, bx + bw * 0.1f, by + bh * 0.65f, bw * 0.8f, 3 * sc, 0, 180); }
        }

        static void PaintKey(Graphics g, float cx, float cy, bool on, Color col, float sc) {
            // Elegant skeleton key: horizontal, loop on right, teeth on left pointing down
            g.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
            float yBump = 0f;
            
            // Loop (Ring) on the right
            float kr = 3f * sc; 
            float kx = cx + 5f * sc, ky = cy + yBump;
            using (var p = new Pen(col, 1.5f * sc))
                g.DrawEllipse(p, kx - kr, ky - kr, kr * 2, kr * 2);
                
            // Loop Inner detail arc (creates a 3D rim effect)
            if (on) {
                using (var p = new Pen(Color.FromArgb(80, 255, 255, 255), 0.75f * sc)) {
                    g.DrawArc(p, kx - kr + 1f * sc, ky - kr + 1f * sc, (kr - 0.5f * sc) * 2, (kr - 0.5f * sc) * 2, -90, 90);
                }
            }
            
            // Shaft (from inside loop extending left)
            using (var p = new Pen(col, 1.5f * sc)) {
                p.StartCap = System.Drawing.Drawing2D.LineCap.Round;
                p.EndCap = System.Drawing.Drawing2D.LineCap.Round;
                g.DrawLine(p, cx + 3.5f * sc, cy + yBump, cx - 8f * sc, cy + yBump);
                
                // Teeth
                using (var p2 = new Pen(col, 1.5f * sc)) {
                    p2.StartCap = System.Drawing.Drawing2D.LineCap.Round;
                    p2.EndCap = System.Drawing.Drawing2D.LineCap.Round;
                    g.DrawLine(p2, cx - 6.5f * sc, cy + yBump, cx - 6.5f * sc, cy + 3.5f * sc + yBump); // Outer tooth
                    g.DrawLine(p2, cx - 3.5f * sc, cy + yBump, cx - 3.5f * sc, cy + 3.5f * sc + yBump); // Inner tooth
                    g.DrawLine(p2, cx - 6.5f * sc, cy + 3.5f * sc + yBump, cx - 5f * sc, cy + 3.5f * sc + yBump); // Bit bridge
                }
            }
            
            if (on) {
                // Glow dot inside the loop
                float er = 1.0f * sc;
                using (var b = new SolidBrush(Color.FromArgb(200, 255, 255, 255)))
                    g.FillEllipse(b, kx - er, ky - er, er * 2, er * 2);
            }
        }
    }
    ...
    public class CardToggle
    {
        ...
        public bool Hover;
        public bool Visible = true; // if false, skip paint and hit-test
        public bool Dimmed  = false; // if true, paint faded and block clicks
        public Panel Card;
        ...
        public void Click() {
            if (Source != null) Source.Checked = !Source.Checked; if (Card != null) Card.Invalidate();
        }
        ...
        public void Paint(Graphics g, Color accent) {
            ...
            Color bg = Dimmed ? Color.FromArgb(35, 35, 35)
                : on ? (Hover ? Color.FromArgb(130, 210, 255) : accent) : (Hover ? Color.FromArgb(90, 90, 90) : Color.FromArgb(55, 55, 55));
            using (var b = new SolidBrush(bg)) {
    ...
    // ── DarkComboBox ──────────────────────────────────────────────────────────────
    // Fully dark-themed ComboBox. Overrides WM_PAINT to repaint the OS-rendered
    // dropdown arrow button so it matches the app's dark palette.
    public class DarkComboBox : ComboBox {
            ...
            if (m.Msg == WM_PAINT) {
                try {
                    using (var g = Graphics.FromHwnd(Handle)) {
                        // Cover the OS-rendered arrow button with a dark version
                        int bw = Dpi.S(22);
                        var btnRect = new Rectangle(Width - bw, 1, bw - 1, Height - 2);
                        using (var bg = new SolidBrush(BackColor))
                            g.FillRectangle(bg, btnRect);
                        // Draw dark arrow
                        int cx = btnRect.X + btnRect.Width / 2;
                        int cy = btnRect.Y + btnRect.Height / 2 - Dpi.S(1);
                        int sz = Dpi.S(4);
                        g.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
                        using (var b = new SolidBrush(DarkTheme.Txt2))
                            g.FillPolygon(b, new[] {
                                new Point(cx - sz, cy),
                                new Point(cx + sz, cy),
                                new Point(cx, cy + sz + Dpi.S(1))
                            });
                        // Re-draw outer border
                        using (var p = new System.Drawing.Pen(DarkTheme.InputBdr))
        ...
        protected override void OnDrawItem(DrawItemEventArgs e) {
            ...
            using (var bg = new SolidBrush(sel ? DarkTheme.InputHover : BackColor))
                e.Graphics.FillRectangle(bg, e.Bounds);
            using (var br = new SolidBrush(sel ? DarkTheme.Txt : DarkTheme.Txt2)) {
                var sf = new System.Drawing.StringFormat {
                    Alignment = StringAlignment.Near,
                    LineAlignment = StringAlignment.Center
                };
                var textRect = new RectangleF(e.Bounds.X + Dpi.S(6), e.Bounds.Y, e.Bounds.Width - Dpi.S(6), e.Bounds.Height);
                e.Graphics.DrawString(Items[e.Index].ToString(), Font, br, textRect, sf);
            }
            if (sel) {
                using (var p = new System.Drawing.Pen(Color.FromArgb(60, DarkTheme.Accent.R, DarkTheme.Accent.G, DarkTheme.Accent.B)))
