...
namespace AngryAudio
{
    ...
    // --- WASAPI Capture interfaces for raw mic input ---
    ...
    public static class Audio
    {
        ...
        /// <summary>
        /// Returns the LOWEST volume level across ALL active capture devices.
        /// If any mic is below the lock target, enforcement corrects ALL of them.
        /// </summary>
            ...
            try
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (minVol < 0f || pct < minVol) minVol = pct;
                    }
                    catch { }
                    finally
                    {
        ...
        // --- Voice Activity: Peak Level Monitoring ---

        /// <summary>
        /// Get the current peak level from the default capture device.
        /// Returns 0.0–1.0 (normalized amplitude). Returns -1 on failure.
        /// The peak meter reads BEFORE endpoint volume/mute attenuation,
        /// so it reports levels even when the mic is muted.
        /// </summary>
        public static float GetMicPeakLevel()
        {
            IAudioMeterInformation meter = null;
            try
            {
                meter = GetMicMeterInfo();
            ...
            {
                Logger.Error("GetMicPeakLevel failed.", ex);
                return -1f;
            }
            finally
            {
        ...
        /// <summary>
        /// Get the IAudioMeterInformation COM object for the default capture device.
        /// Caller MUST release via Marshal.ReleaseComObject when done.
        /// For long-lived monitoring, hold this reference and call GetPeakValue repeatedly.
        /// </summary>
        internal static IAudioMeterInformation GetMicMeterInfo()
        {
            IMMDeviceEnumerator enumerator = null;
            IMMDevice device = null;
            try
            {
                ...
                object iface;
                hr = device.Activate(IID_IAudioMeterInformation, 1 /* CLSCTX_INPROC_SERVER */, IntPtr.Zero, out iface);
                if (hr != 0 || iface == null) return null;

                return (IAudioMeterInformation)iface;
            }
            catch (Exception ex)
            {
                Logger.Error("GetMicMeterInfo failed.", ex);
                return null;
            }
            finally
            {
        ...
        /// <summary>Opens a raw WASAPI capture stream on the default mic. Bypasses mute/volume.</summary>
        public static bool OpenRawCapture(out IAudioClient audioClient, out IAudioCaptureClient captureClient, out int channels, out int bitsPerSample, out int blockAlign)
        {
            audioClient = null; captureClient = null; channels = 0; bitsPerSample = 0; blockAlign = 0;
            IMMDeviceEnumerator enumerator = null; IMMDevice device = null; IntPtr formatPtr = IntPtr.Zero;
            try {
                enumerator = (IMMDeviceEnumerator)new MMDeviceEnumeratorClass();
                int hr = enumerator.GetDefaultAudioEndpoint(EDataFlow.eCapture, ERole.eConsole, out device);
                if (hr != 0 || device == null) return false;
                object iface;
                hr = device.Activate(IID_IAudioClient, 23, IntPtr.Zero, out iface);
                if (hr != 0 || iface == null) return false;
                audioClient = (IAudioClient)iface;
                hr = audioClient.GetMixFormat(out formatPtr);
                if (hr != 0 || formatPtr == IntPtr.Zero) { audioClient = null; return false; }
                var fmt = (WAVEFORMATEX)Marshal.PtrToStructure(formatPtr, typeof(WAVEFORMATEX));
                channels = fmt.nChannels; bitsPerSample = fmt.wBitsPerSample; blockAlign = fmt.nBlockAlign;
                var empty = Guid.Empty;
                hr = audioClient.Initialize(0, 0, 2000000, 0, formatPtr, ref empty);
                if (hr != 0) { audioClient = null; return false; }
                object capIface;
                hr = audioClient.GetService(IID_IAudioCaptureClient, out capIface);
                if (hr != 0 || capIface == null) { audioClient = null; return false; }
                captureClient = (IAudioCaptureClient)capIface;
                hr = audioClient.Start();
                if (hr != 0) { captureClient = null; audioClient = null; return false; }
                Logger.Info("Raw WASAPI capture opened: " + fmt.nSamplesPerSec + "Hz, " + channels + "ch, " + bitsPerSample + "bit");
                return true;
            } catch (Exception ex) {
                Logger.Error("OpenRawCapture failed.", ex);
                audioClient = null; captureClient = null; return false;
            } finally {
        ...
        /// <summary>Read raw audio from capture stream and return peak level (0.0-1.0).</summary>
        public static float ReadRawPeak(IAudioCaptureClient captureClient, int channels, int bitsPerSample, int blockAlign)
        {
            if (captureClient == null) return 0f;
            float maxPeak = 0f;
            try {
                uint packetSize;
                captureClient.GetNextPacketSize(out packetSize);
                while (packetSize > 0) {
                    IntPtr dataPtr; uint numFrames, flags; ulong devPos, qpcPos;
                    int hr = captureClient.GetBuffer(out dataPtr, out numFrames, out flags, out devPos, out qpcPos);
                    if (hr != 0) break;
                    if ((flags & 2) == 0 && numFrames > 0) { // not AUDCLNT_BUFFERFLAGS_SILENT
                        int totalBytes = (int)(numFrames * blockAlign);
                        if (bitsPerSample == 32) {
                            int cnt = Math.Min(totalBytes / 4, 4096);
                            float[] buf = new float[cnt];
                            Marshal.Copy(dataPtr, buf, 0, cnt);
                            for (int i = 0; i < cnt; i++) {
                                float a = Math.Abs(buf[i]); if (a > maxPeak) maxPeak = a;
                            }
                        } else if (bitsPerSample == 16) {
                            int cnt = Math.Min(totalBytes / 2, 4096);
                            short[] buf = new short[cnt];
                            Marshal.Copy(dataPtr, buf, 0, cnt);
                            for (int i = 0; i < cnt; i++) {
                                float a = Math.Abs(buf[i] / 32768f); if (a > maxPeak) maxPeak = a;
                            }
                        }
                    }
            ...
            return Math.Min(1f, maxPeak);
        }

        /// <summary>Stop and release raw capture stream.</summary>
        public static void CloseRawCapture(ref IAudioClient audioClient, ref IAudioCaptureClient captureClient)
        {
            try { if (audioClient != null) audioClient.Stop(); } catch { }
            try { if (audioClient != null) audioClient.Reset(); } catch { }
            try { if (captureClient != null && Marshal.IsComObject(captureClient)) Marshal.ReleaseComObject(captureClient); } catch { }
            try { if (audioClient != null && Marshal.IsComObject(audioClient)) Marshal.ReleaseComObject(audioClient); } catch { }
            captureClient = null; audioClient = null;
        }
        ...
        private static readonly string EqApoConfigPath =
            System.IO.Path.Combine(
                System.Environment.GetFolderPath(System.Environment.SpecialFolder.ProgramFiles),
                @"EqualizerAPO\config\config.txt");

        private static readonly int[] EQ_FREQ_HZ = { 31, 62, 125, 250, 500, 1000, 2000, 4000, 8000, 16000 };

        /// <summary>Returns true if Equalizer APO is installed (config file exists).</summary>
        public static bool IsEqualizerAPOInstalled()
        {
            try { return System.IO.File.Exists(EqApoConfigPath); }
            catch { return false; }
        }

        /// <summary>
        /// Writes the Equalizer APO config file from a 10-element float array (0.0-1.0, 0.5=flat=0dB).
        /// Equalizer APO watches this file and reloads live - no service restart needed.
        /// Returns null on success, or an error string on failure.
        /// </summary>
        public static string WriteEqualizerAPOConfig(float[] bands)
        {
            try
            {
                if (bands == null || bands.Length != 10)
                    return "Invalid band data";
                if (!System.IO.File.Exists(EqApoConfigPath))
                    return "Equalizer APO not installed";

                float maxGain = 0f;
                for (int i = 0; i < 10; i++)
                {
                    float db = (bands[i] - 0.5f) * 24f;
                    if (db > maxGain) maxGain = db;
                }

                var sb = new System.Text.StringBuilder();
                sb.AppendLine("# Angry Audio EQ - auto-generated, do not edit manually");
                sb.AppendLine(maxGain > 0f
                    ? string.Format("Preamp: -{0:F1} dB", maxGain)
                    : "Preamp: 0 dB");
                ...
                for (int i = 0; i < 10; i++)
                {
                    ...
                    sb.AppendLine(string.Format(
                        ...
                        "Filter {0}: ON PK Fc {1} Hz Gain {2:F1} dB Q 1.41",
                        i + 1, EQ_FREQ_HZ[i], db));
                }

                System.IO.File.WriteAllText(EqApoConfigPath, sb.ToString(), System.Text.Encoding.UTF8);
                return null;
            }
            catch (Exception ex) { return ex.Message; }
        }

        // Compression is handled separately via ApplyLoudnessEqualization (FxProperties), not APO config.
        public static string WriteEqualizerAPOConfig(float[] bands, bool compression, int releaseSec)
        {
            return WriteEqualizerAPOConfig(bands);
        }

        /// <summary>
        /// Enables or disables Windows Loudness Equalization on the default render device via FxProperties,
        /// then restarts the Windows Audio service so changes take effect.
        /// Returns null on success, or an error string on failure.
        /// </summary>
        public static string ApplyLoudnessEqualization(bool enable)
        {
            try
            {
                string deviceId = GetSpeakerDeviceId();
                if (string.IsNullOrEmpty(deviceId))
                    return "No audio output device found";

                int braceIdx = deviceId.LastIndexOf('{');
                if (braceIdx < 0)
                    return "Unexpected device ID format: " + deviceId;
                string guid = deviceId.Substring(braceIdx);

                string regPath = @"SOFTWARE\Microsoft\Windows\CurrentVersion\MMDevices\Audio\Render\"
                    + guid + @"\FxProperties";

                using (var key = Microsoft.Win32.Registry.LocalMachine.CreateSubKey(regPath,
                    Microsoft.Win32.RegistryKeyPermissionCheck.ReadWriteSubTree))
                {
                    if (key == null) return "Cannot open FxProperties registry key";

                    // Loudness Equalization APO CLSID: {4DC2107E-CB11-45FA-B05D-3B9C6C14AAAE}
                    if (enable)
                    {
                        key.SetValue("{d04e05a6-594b-4fb6-a80d-01af5eed7d1d},1",
                            "{62dc1a93-ae24-464c-a43e-452f824c4250}", Microsoft.Win32.RegistryValueKind.String);
                        key.SetValue("{d04e05a6-594b-4fb6-a80d-01af5eed7d1d},2",
                            "{637c490d-eee3-4c0a-973f-371958802da2}", Microsoft.Win32.RegistryValueKind.String);
                        key.SetValue("{d04e05a6-594b-4fb6-a80d-01af5eed7d1d},3",
                            "{5860E1C5-F95C-4a7a-8EC8-8AEF24F379A1}", Microsoft.Win32.RegistryValueKind.String);
                    }

                    // Loudness EQ enable flag — same FxProperties slot, different flag CLSID
                    byte[] flagBytes = { 0x03,0x00,0x00,0x00, 0x01,0x00,0x00,0x00,
                        enable ? (byte)0x02 : (byte)0x00, 0x00,0x00,0x00 };
                    key.SetValue("{4DC2107E-CB11-45FA-B05D-3B9C6C14AAAE},1",
                        flagBytes, Microsoft.Win32.RegistryValueKind.Binary);
                }
            ...
            {
                Logger.Error("ApplyLoudnessEqualization failed", ex);
                return ex.Message;
            }
        }
        ...
        /// <summary>Restarts the Windows Audio service (audiosrv). Requires admin rights.</summary>
